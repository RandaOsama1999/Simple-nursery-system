...
border:1px solid black;
    ...
    <title>Smart Kids nursery</title>
        ...
        $Message="SELECT * FROM n_system.comments WHERE MailT='$email' AND Pcomment IS NOT NULL";
        $result= mysqli_query($conn,$Message);
       echo'<h2 style="text-align:center; color: rgba(196, 90, 143, 0.836);">'.'Comments From Parents';
        while($row = $result->fetch_assoc()) 
        { 
            echo '<h2 style="text-align:center; color: rgb(27, 73, 92) ;">'.'Mail of the Parent:'
            . $row["MailP"] .'<br>'.' Parent Comment:'. $row["Pcomment"].'<br>'.'Date: '. $row["Date"].'<br>'.'Time: '. $row["Time"]. '</h2>';
            echo "<br>";
        }
        ?>
