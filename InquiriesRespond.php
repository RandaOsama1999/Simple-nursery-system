...
border:1px solid black;
    ...
    <title>Smart Kids nursery</title>
        ...
        $Message="SELECT * FROM n_system.comments WHERE MailP='$email' AND Tcomment IS NOT NULL";
        $result= mysqli_query($conn,$Message);
       echo'<h2 style="text-align:center; color: rgba(196, 90, 143, 0.836);">'.'Comments From Teachers';
        while($row = $result->fetch_assoc()) 
        {
            echo '<h2 style="text-align:center; color: rgba(196, 90, 143, 0.836) ;">'.'Mail of the Teacher:'
            . $row["MailT"] .'<br>'.' Teacher Comment:'. $row["Tcomment"].'<br>'.'Date: '. $row["Date"].'<br>'.'Time: '. $row["Time"].'</h2>';
            echo "<br>";
        }
        ?>
       </div>
