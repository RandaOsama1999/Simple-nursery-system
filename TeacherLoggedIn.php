<?php 
  ...
  {
    header("location:TeacherEditteacher.php");
  }
  ...
  {
    header('location: ReceivecommentFromParent.php');
  }
  if(isset($_GET['Sendcomment']))
  {
    header('location: SendcommentFromTheTeacher.php');
  }
?>
<html>
    <head>
...
li button.active {
    ...
    <title>Smart Kids nursery</title>
        ...
        if (isset($_SESSION['email']))
        {
            ...
            if($row==true)
            {
                echo '<h2 style="text-align:center; color: rgba(196, 90, 143, 0.836);">Welcome ' . $row["FirstName"] . "!</h2>"; 
                //echo '<h2 style="text-align:center; color: rgba(196, 90, 143, 0.836);">Welcome ' . $username .' '. $row["LastName"].' '. $row["FamilyName"]. '!</h2>';
                echo "<br>";
                /*echo '<h4 style="text-align:center;"> Date of birth: ' .  $row["DateOfBirth"] .', Age: '. $row["Age"]. '</h4> ';
                echo '<h4 style="text-align:center;"> Gender: '. $row["Gender"].'</h4>';
                echo '<h4 style="text-align:center;"> Father Name: '. $row["FatherName"].', Mother Name: '.$row["MotherName"].'</h4>';
                echo '<h4 style="text-align:center;"> Mobile Number: 0' .  $row["Mobile"] .'</h4> ';
                echo '<h4 style="text-align:center;"> Second Mobile Number: 0' .  $row["MobileTwo"] .'</h4> ';
                echo '<h4 style="text-align:center;"> Home Number: '. $row["Home"]. '</h4> ';
                echo '<h4 style="text-align:center;"> Parent Mail: '. $row["ParentMail"]. '</h4> ';
                echo '<h4 style="text-align:center;"> Address: '. $row["Address"]. '</h4> ';
                echo '<h4 style="text-align:center;"> Hobby: '. $row["Hobby"]. '</h4> ';
                if($row["School"]==NULL){
                    echo '<h4 style="text-align:center;"> Bus subscription: '. $row["Bus"]. '</h4> ';
                    echo "<br>";
                }
                else{
                    echo '<h4 style="text-align:center;"> School: '. $row["School"]. '</h4> ';
                    echo '<h4 style="text-align:center;"> Bus subscription: '. $row["Bus"]. '</h4> ';
                    echo "<br>";
                }
                */
        ...
        <form  method="get" >
        <ul>
            <li><button name="EditInfo">Edit your Info</button></li>
            <li><button name="schedule">View Schedule</button></li>
            <!--<li><button name="Inquiry">Recieve interview info</button></li>-->
            <li><button name="Recievecomment">Recieve comments</button></li>
            <li><button name="Sendcomment">Send comments</button></li>
            <!--<li><button name="EditDeleteChild">Recieve interview info</button></li>-->
