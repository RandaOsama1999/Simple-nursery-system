...
border:1px solid black;
    ...
    <title>Smart Kids nursery</title>
        ...
        <?php
            ...
            $sql = "SELECT * FROM n_system.manager WHERE ManagerMail='$email'";
            $result = $conn->query($sql);
            while($rowTwo = $result->fetch_assoc()){
                if($rowTwo==true)
                {
                    $teachermail = $_GET['teachermail'];
                    $sql2 = "SELECT * FROM n_system.users WHERE Mail='$teachermail' AND userType_ID=3";
                    $resultTwo = $conn->query($sql2);
                    $sql1="SELECT * FROM n_system.teacher WHERE TeacherMail='$teachermail'";
                    ...
                    if(isset($_POST['save'])){
                        ...
                        Mobile='$Mob',MobileTwo='$MobTwo', Home='$Home', Address='$Addr', Password='$UserP' WHERE Mail='$teachermail'";

                        $query1="UPDATE n_system.teacher SET Schedule_ID='$Schedule_ID' WHERE TeacherMail='$teachermail'";
...
<script>
        ...
        <h2 style="text-align:center; color: rgba(196, 90, 143, 0.836);">Edit Teacher's Info</h2>
        <br>
