<?php 
  ...
  if(isset($_POST['save']))
  {
    header('location: EditSubmittedInfo.php');
  }
?>
<html>
    <head>
...
border:1px solid black;
    ...
    <title>Smart Kids nursery</title>
        ...
        <?php
            ...
            while($rowTwo = $result->fetch_assoc())
            {
                if($rowTwo==true)
                {
                            ...
                            $FamilyName=$rowTwo['FamilyName'];
                            $DateOfBirth=$rowTwo['DateOfBirth'];
                            $Mobile=$rowTwo['Mobile'];
                            $MobileTwo=$rowTwo['MobileTwo'];
                            $Home=$rowTwo['Home'];
                            $Address=$rowTwo['Address'];
                            $UserP=$rowTwo["Password"];
                 }  
            }
            $sqlone = " SELECT Hobby, School,Parent_ID FROM n_system.child WHERE ChildMail='$email' AND State_ID=1";
            ...
            {  
              if($rowone==true)
               {
                $Parent_ID=$rowone["Parent_ID"];
                   $Hobby=$rowone['Hobby'];
                   $School=$rowone['School'];
                }
            }

          
           $s1="SELECT * FROM n_system.parent WHERE Parent_ID='$Parent_ID'";
            
            $resultFour = $conn->query($s1);
             ...
             while($rowfour= $resultFour->fetch_assoc())
                 {  
                    if($rowfour==true)
                   {
                        ...
                        $FirstNameP=$rowfour['FirstName'];
                        $LastNameP=$rowfour['LastName'];
                        $FamilyNameP=$rowfour['FamilyName'];
                        $DateOfBirthP=$rowfour['DateOfBirth'];
                        $Job=$rowfour['Job'];
                        $PersonalMail=$rowfour['PersonalMail'];
                   }
            }
            ...
            if(isset($_POST['save']))
            {
                      ...
                      $School=$_POST['School'];
                        ...
                        $FamilyNameP=$_POST['FamilyNameP'];
                        $DateOfBirthP=$_POST['DateOfBirthP'];
                        date_default_timezone_set("Africa/Cairo");
                        $d=strtotime("today");
                        $date= date("Y-m-d", $d);
                        $diff = date_diff(date_create($DateOfBirthP), date_create($date));
                        ...
                        $sqlone = " SELECT Parent_ID FROM n_system.child WHERE ChildMail='$email'";
                        $resultone = $conn->query($sqlone);
                        ...
                        {  
                            ...
                            {
                                $Parent_ID=$rowone['Parent_ID'];
                                $sqlFive="UPDATE n_system.parent SET FirstName='$FirstNameP',LastName='$LastNameP'
                                ,FamilyName='$FamilyNameP',DateOfBirth='$DateOfBirthP',Age='$AgeP'
        ...
        <br>
        <p style='text-align:center; font-size:medium;'>FirstName: <input type='text' name='FirstNameP' pattern="[A-Za-z]{3,15}" title="In Lettres more than 3 letters, less than 8" value='<?php echo $FirstNameP; ?>'><p>
        <p style='text-align:center; font-size:medium;'>LastName: <input type='text' name='LastNameP' pattern="[A-Za-z]{3,15}" title="In Lettres more than 3 letters, less than 8" value='<?php echo $LastNameP; ?>'><p>
        <p style='text-align:center; font-size:medium;'>FamilyName: <input type='text' name='FamilyNameP' pattern="[A-Za-z]{3,15}" title="In Lettres more than 3 letters, less than 8" value='<?php echo $FamilyNameP; ?>'><p>
        <p style='text-align:center; font-size:medium;'>Date Of Birth: <input type='date' name='DateOfBirthP' value='<?php echo $DateOfBirthP; ?>'><p>
        ...
        <p style='text-align:center; font-size:medium;'>Personal Mail: <input type='text' name='PersonalMail' value='<?php echo $PersonalMail; ?>'><p>
        <br>
        <p style='text-align:center; font-size:medium;'>Password: <input type='text' name='Password' value='<?php echo $UserP; ?>'><p>
        <br>
        <button class="btn" name="save" style="margin-bottom:1%;">Edit</button>
    </form>
